...
def predict_ml_win_probability(surge_ratio: float, cmf: float, obv_trend: str) -> Dict[str, Any]:
    """Predict ML Win Probability for a stock signal based on learned feature weights."""
    obv_val = 1.0 if obv_trend == "RISING" else 0.0
    
    # Mathematical Sigmoid / Random Forest Decision Probability approximation
    score = (surge_ratio * 0.25) + (cmf * 1.5) + (obv_val * 0.3)
    prob = 1.0 / (1.0 + np.exp(-score))
    win_pct = round(min(98.0, max(52.0, prob * 100.0)), 1)
    ...
    return {
        ...
        "isHighProbability": win_pct >= 80.0
    }
...
def train_brain_model(stocks_df_dict: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
    ...
    for symbol, df in stocks_df_dict.items():
        ...
        for i in range(20, len(df) - 5):
            ...
            # Save into SQLite memory DB
            cursor.execute(
                "INSERT INTO signal_memory (symbol, date, surge_ratio, cmf, obv_trend, price_change_pct, outcome_win) VALUES (?, ?, ?, ?, ?, ?, ?)",
